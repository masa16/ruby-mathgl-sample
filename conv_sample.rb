...
class ConvSample
  def initialize
    ...
    @n_indent = 0
    @out_dir = "samples/"
    @raw_path = "https://raw.github.com/masa16/ruby-mathgl-sample/master/"
...
"
    ...
    single_dir = "single_page/"
    FileUtils.mkdir_p(single_dir)
    File.open(single_dir+'README.md','w') do |f|
      f.write '# Ruby/MathGL sample codes'
      @single.keys.sort.each do |k|
        f.write @single[k]
      end
    end

    File.open(@out_dir+'mkimg.rb','w') do |f|
      f.write "
